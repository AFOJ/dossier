...
import { ModalProvider, type ModalContentProps, useModal } from "@/components/modal"
...
function ModalControls() {
  ...
  return (
    <>
      ...
      <button type="button" onClick={() => persistentModal.open("Persistent modal")}>
        Open persistent
      </button>
    </>
  )
...
describe("ModalProvider", () => {
  it("stacks modals by opening order and traps focus in the top modal", async () => {
    ...
    expect(document.activeElement).toBe(screen.getByRole("button", { name: "Close Second modal" }))
  ...
  it("honours dismissal options supplied to useModal", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Close Persistent modal" }))
