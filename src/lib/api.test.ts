      ...
      json: () =>
        Promise.resolve({
          error: {
            ...
            details: [{ path: "/sections/0/text", message: "too_small" }],
          },
        }),
    }))

    const error = await processResume({ title: "x", sections: [] }).catch((cause) => cause)

    expect(error).toBeInstanceOf(ApiError)
    expect(error.code).toBe("VALIDATION_ERROR")
    expect(error.message).toBe("Invalid request")
    expect(error.details).toEqual([{ path: "/sections/0/text", message: "too_small" }])
  })
...
describe("getErrorFeedback", () => {
  ...
  it("shows LaTeX log excerpts verbatim for compile errors", () => {
    ...
    expect(getErrorFeedback(error)).toEqual(["! Undefined control sequence.\nl.42 \\foo"])
  })
...
describe("formatApiError", () => {
  ...
  it("appends string details verbatim", () => {
    const error = new ApiError("COMPILE_ERROR", "Compilation failed", ["! LaTeX Error"])
