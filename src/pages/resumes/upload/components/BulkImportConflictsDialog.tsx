...
function describeContact(resume: Resume): string {
  return resume.contact?.full_name ?? "Synced with profile"
}
...
export function BulkImportConflictsDialog(
  ...
  // Decisions are keyed by item position, not resume id: the same resume
  // can appear more than once in a batch (e.g. a file staged twice) and
  // each occurrence must be decided independently.
  ...
  for (const [positionKey, decision] of Object.entries(decisions)) {
    if (decision === "overwrite") {
      const conflictId = items[Number(positionKey)].incomingResumeId
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      toast.error("Failed to import resumes", "Please try again.")
      setIsApplying(false)
    }
  }
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="rounded-lg border border-gray-200 bg-gray-50 px-4 py-2">
        ...
        <dl className="divide-y divide-gray-200">
          <ComparisonRow
            label="Title"
            existing={currentItem.existingResume.title}
            incoming={currentItem.incomingResume.title}
          />
          <ComparisonRow
            label="Sections"
            existing={`${currentItem.existingResume.sections.length} sections`}
            incoming={`${currentItem.incomingResume.sections.length} sections`}
          />
          <ComparisonRow
            label="Last updated"
            existing={formatDate(currentItem.existingResume.updatedAt)}
            incoming={formatDate(currentItem.incomingResume.updatedAt)}
          />
          <ComparisonRow
            label="Contact"
            existing={describeContact(currentItem.existingResume)}
            incoming={describeContact(currentItem.incomingResume)}
          />
        </dl>
      </div>
      ...
      {duplicateOverwriteCount > 0 && (
        <Callout intent="yellow" role="status">
          Heads up: {duplicateOverwriteCount} items target the same resume with Overwrite. Only the
          last in the list will remain. Use Keep existing or Create copy for the others.
        </Callout>
      )}
