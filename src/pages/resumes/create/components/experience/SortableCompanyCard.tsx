...
export function SortableCompanyCard(props: Readonly<SortableCompanyCardProps>) {
  ...
  const [bodyId] = useState(() => `company-body-${crypto.randomUUID()}`)
    ...
    >
      <div className="flex items-center gap-2 p-3 pb-2">
        <button
          ...
          aria-label={`Reorder ${label} company`}
          className={cn(
        ...
        <div className="flex items-center gap-1">
          <Button
            ...
            icon={Trash}
            onClick={onRemove}
            aria-label={`Remove ${label} company`}
            className="w-9 px-0"
          />
          <Button
            type="button"
            ...
            aria-label={`${isOpen ? "Collapse" : "Expand"} ${label} company`}
