    ...
    <ModalProvider>
      <MemoryRouter initialEntries={["/resumes"]}>
        <Routes>
          ...
          <Route path="/resumes/:resumeId/edit" element={<div>Edit resume page</div>} />
        </Routes>
      </MemoryRouter>
    </ModalProvider>,
  )
...
describe("ResumesListPage", () => {
  ...
  it("duplicates a resume with a copy title and its sections", async () => {
    const user = userEvent.setup()
    const sections: ResumeSection[] = [{ type: "paragraph", title: "Hello", text: "Hello" }]
    renderPage(
      makeTableState({
        totalCount: 1,
        pageItems: [makeResume({ id: "1", title: "Frontend Engineer", sections })],
      }),
    )
  ...
  it("confirms before deleting a resume", async () => {
    ...
    renderPage(
      makeTableState({
        totalCount: 1,
        pageItems: [makeResume({ id: "delete-me", title: "Frontend Engineer" })],
      }),
    )

    await user.click(await screen.findByRole("button", { name: "Delete" }))

    expect(await screen.findByRole("dialog")).toHaveTextContent("Delete resume?")
