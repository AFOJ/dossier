...
describe("useEditProfileForm", () => {
  ...
  it("saves the mapped payload, resets dirty state and revalidates on success", async () => {
    ...
    expect(result.current.isDirty).toBe(false)
    expect(mockToast.success).toHaveBeenCalledWith("Profile saved", "Your changes have been saved.")
  ...
  it("sets a root error and an error toast when saving fails", async () => {
    ...
    expect(result.current.isDirty).toBe(true)
    expect(mockToast.error).toHaveBeenCalledWith("Failed to save profile", "Please try again.")
