...
vi.mock("@/lib/download", () => ({
  downloadJson: vi.fn(),
  getExportFilename: vi.fn((kind: string) => `dossier-${kind}-export-2026-08-23.json`),
}))
...
describe("ProfilePage", () => {
  ...
  it("shows an error toast when exporting fails", async () => {
    ...
    await waitFor(() => {
      expect(mockToast.error).toHaveBeenCalledWith("Failed to export data", "Please try again.")
    })
  ...
  it("keeps the dialog open and shows an error when deletion fails", async () => {
    ...
    expect(
      await screen.findByText("Unable to delete your profile. Please try again."),
    ).toBeInTheDocument()
    expect(mockToast.error).toHaveBeenCalledWith("Failed to delete profile", "Please try again.")
