...
import { useUploadCoverLetter } from "./hooks/useUploadCoverLetter"
import type { BatchParsedItem, StagedFile } from "./hooks/useUploadCoverLetter"

interface UploadCoverLetterJsonProps {
...
export function UploadCoverLetterJson(props: Readonly<UploadCoverLetterJsonProps>) {
  ...
  const {
    ...
    openFileDialog,
    fileInputRef,
  } = useUploadCoverLetter({ onBatchParsed })
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setImportError(
        "Import failed. Some cover letters may not have been imported. Please try again.",
      )
      toast.error(
        "Import failed",
        "Some cover letters may not have been imported. Please try again.",
      )
    } finally {
    ...
    <div className="flex flex-col gap-4">
      <div className="relative">
        ...
        <div
          ...
          onDrop={onDrop}
          role="region"
          aria-label="Upload cover letter files"
        >
          <IsometricFileOpen aria-hidden="true" />
          <Heading3 className="mt-4">Upload cover letters</Heading3>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-t border-gray-200 px-4 py-3">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={handleImport} disabled={busy || validCount === 0}>
                {isImporting
                  ? "Importing…"
                  : `Import ${validCount} cover letter${validCount === 1 ? "" : "s"}`}
              </Button>
            </div>
          </div>
        </div>
